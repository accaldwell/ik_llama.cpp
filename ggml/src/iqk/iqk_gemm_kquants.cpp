...
// Unified AVX512 kernel for K-quant x q8_2_x4 matrix multiply.
// Two 256-bit sub-blocks (j=0, j=1) are packed into 512-bit lanes and dispatched
// through a single vpdpbusd path. Compile-time traits select the variant:
//   is_signed      : Q6 path — signed dot product via us[]+sign_epi8; scales via make_scales()+shuffle.
//                    Q4/Q5 path — unsigned dot product; scales derived from utmp; dmin/mins accumulated.
//   has_prepare_x4 : Dequantizer provides a single-call prepare_x4(i, v512_*) that
//                    returns the 512-bit left-operand directly (Q5_K AVX512 variant).
template <typename Dequantizer, int nrc_y, bool is_signed, bool has_prepare_x4 = false>
static void mul_mat_kquant_q8_2_X4_AVX512(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    assert(n % QK_K == 0);
    const int nb = n / QK_K;
    ...
    __m256  scales[2];
    float   d8[8*nrc_y];
    [[maybe_unused]] uint32_t utmp[4];
    [[maybe_unused]] __m256i us[4];
    [[maybe_unused]] __m256i shuff;
    if constexpr (is_signed) {
        uint8_t k_shuff[32] = {0, 1, 4, 5, 8, 9, 12, 13, 2, 3, 6, 7, 10, 11, 14, 15, 0, 1, 4, 5, 8, 9, 12, 13, 2, 3, 6, 7, 10, 11, 14, 15};
        shuff = _mm256_loadu_si256((const __m256i *)k_shuff);
    }
        ...
        for (int i = 0; i < nb; ++i) {
            ...
            if constexpr (!is_signed) {
                ...
                for (int iy = 0; iy < nrc_y; ++iy) {
                    ...
                    auto myi  = MM256_SET_M128I(m4_2, m4_1);
                    auto my   = _mm256_mul_ps(dy, _mm256_cvtepi32_ps(myi));
                    // Fold mins into low lane only; high lane is added via the final horizontal sum.
            ...
            [[maybe_unused]] __m256i bits_j0[4], bits_j1[4];

            if constexpr (has_prepare_x4) {
                ...
                bits_j0[0] = deq.bits.values[0]; bits_j0[1] = deq.bits.values[1];
                bits_j0[2] = deq.bits.values[2]; bits_j0[3] = deq.bits.values[3];
...
#ifdef HAVE_FANCY_SIMD
#define SET_KQUANT_AVX512(Deq, is_signed, has_prepare_x4) \
            kernels[0] = mul_mat_kquant_q8_2_X4_AVX512<Deq, 1, is_signed, has_prepare_x4>; \
            kernels[1] = mul_mat_kquant_q8_2_X4_AVX512<Deq, 2, is_signed, has_prepare_x4>; \
            kernels[2] = mul_mat_kquant_q8_2_X4_AVX512<Deq, 3, is_signed, has_prepare_x4>; \
            kernels[3] = mul_mat_kquant_q8_2_X4_AVX512<Deq, 4, is_signed, has_prepare_x4>; \
            kernels[4] = mul_mat_kquant_q8_2_X4_AVX512<Deq, 5, is_signed, has_prepare_x4>; \
            kernels[5] = mul_mat_kquant_q8_2_X4_AVX512<Deq, 6, is_signed, has_prepare_x4>; \
            kernels[6] = mul_mat_kquant_q8_2_X4_AVX512<Deq, 7, is_signed, has_prepare_x4>; \
            kernels[7] = mul_mat_kquant_q8_2_X4_AVX512<Deq, 8, is_signed, has_prepare_x4>;
#endif
    ...
    switch (typeA) {
        ...
        case GGML_TYPE_Q4_K:
#ifdef HAVE_FANCY_SIMD
            SET_KQUANT_AVX512(DequantizerQ4K_AVX2, false, false);
#else
        ...
        case GGML_TYPE_Q5_K:
#ifdef HAVE_FANCY_SIMD
            SET_KQUANT_AVX512(DequantizerQ5K_AVX512, false, true);
#else
        ...
        case GGML_TYPE_Q6_K:
#ifdef HAVE_FANCY_SIMD
            SET_KQUANT_AVX512(DequantizerQ6K_AVX2, true, false);
#else
